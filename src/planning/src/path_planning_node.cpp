...
public:
    PathPlanningNode() : Node("path_planning")
    {
        ...
        // 设置规划参数
        planning_horizon_ = 5.0;  // 5秒的规划时间范围
        grid_resolution_ = 0.5;   // 0.5米的网格分辨率
    }

private:
    void obstacleCallback(const auto_drive_msgs::msg::Obstacle::SharedPtr msg)
    {
        RCLCPP_INFO(this->get_logger(), "Received obstacle with ID: %d at position (%.2f, %.2f)", 
        ...
        std::vector<std::pair<int, int>> path = aStar(start_x, start_y, end_x, end_y);

        for (const auto& [x, y] : path)
        {
            geometry_msgs::msg::PoseStamped pose;
            pose.header = path_msg->header;
            pose.pose.position.x = x * grid_resolution_;
            pose.pose.position.y = y * grid_resolution_;
            pose.pose.position.z = 0.0;
            pose.pose.orientation.w = 1.0;
            path_msg->poses.push_back(pose);
        }
        ...
        path_pub_->publish(std::move(path_msg));
        RCLCPP_INFO(this->get_logger(), "Published path with A* algorithm considering obstacle velocities.");
    }

    void updateObstacleInfo(const auto_drive_msgs::msg::Obstacle::SharedPtr msg)
    {
        int grid_x = static_cast<int>(msg->position.x / grid_resolution_);
        int grid_y = static_cast<int>(msg->position.y / grid_resolution_);
        obstacles_[msg->id] = {grid_x, grid_y, msg->velocity.x, msg->velocity.y};
    }

    bool isCollision(int x, int y, double time)
    {
        for (const auto& [id, obstacle] : obstacles_)
        {
            int future_x = obstacle.x + static_cast<int>(obstacle.vx * time / grid_resolution_);
            int future_y = obstacle.y + static_cast<int>(obstacle.vy * time / grid_resolution_);
            if (x == future_x && y == future_y)
            {
                return true;
            }
        }
    ...
    std::vector<std::pair<int, int>> aStar(int start_x, int start_y, int end_x, int end_y)
    {
        auto heuristic = [](int x1, int y1, int x2, int y2) {
            return std::hypot(x2 - x1, y2 - y1);
        };
        ...
        AStarNode start(start_x, start_y, 0.0, heuristic(start_x, start_y, end_x, end_y));
        open_list.push(start);
        all_nodes[start_x * 1000 + start_y] = start;
        ...
        while (!open_list.empty())
        {
            AStarNode current = open_list.top();
            open_list.pop();
            int current_id = current.x * 1000 + current.y;
            ...
            for (auto& direction : directions)
            {
                int new_x = current.x + direction[0];
                int new_y = current.y + direction[1];
                int neighbor_id = new_x * 1000 + new_y;
                ...
                double time = current.g * grid_resolution_ / 1.0;  // 假设速度为1m/s
                if (isCollision(new_x, new_y, time))
                {
                    continue;
                }
    ...
    rclcpp::Subscription<auto_drive_msgs::msg::Obstacle>::SharedPtr obstacle_sub_;
    rclcpp::Publisher<auto_drive_msgs::msg::Path>::SharedPtr path_pub_;
    std::unordered_map<int, ObstacleInfo> obstacles_;
